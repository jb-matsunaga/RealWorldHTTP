...

TLS1.2にはセッション再開機能(Session resumption)があり、最初のハンドシェイクで以前使っていたセッションID(32ビットの数値)を送ることで、その後の鍵交換が省略され1RTTでセッションが再開する。1.3では事前交通鍵(PSK)を共有しておくことで、0RTTで最初のリクエストから情報を送信できるようになる。

TLS1.3では鍵交換と秘密鍵暗号が分離されたため、最初のClient Helloでクライアント側から鍵の交換が行えるようになる。通信が1報復減り、1RTTで認証が完了する。

今後、、、1回の通信における負荷はさらに減っていくことでになる。
